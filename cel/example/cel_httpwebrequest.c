...
//static CelHttpServeContext httpweb_ctx;
static CelHttpWebRequest *client;
...
void httpwebclient_close(CelHttpWebRequest *client, CelAsyncResult *result)
{
...
int httpwebclient_test(int argc, TCHAR *argv[])
{
    ...
    //    return -1;
    //}
    client = cel_httpwebrequest_new(NULL);

    cel_httpwebrequest_set_nonblock(client, 1);
    cel_eventloop_add_channel(&evt_loop, cel_httpwebrequest_get_channel(client), NULL);
    ...
    cel_httpwebrequest_async_execute_request(client, httpwebclient_close);
