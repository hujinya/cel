                                           ...
                                           CelIpAddr *from, struct timeval *now)
{
    ...
    {
        CEL_SETERR((CEL_ERR_LIB, _T("Invalid checksum, len %d, from %s."), 
    ...
    for (i = 0; i < dev_msg->n_group; i++, grp_msg++)
    {
        ...
        {
            CEL_SETERR((CEL_ERR_LIB, _T("Invalid group message, dev %s, grp %p[id %d], meb %p[priority %d], from %s."), 
...
static int cel_hagroup_transition_state(CelHaGroup *ha_grp, struct timeval *now)
{
    ...
    default:
        ...
        CEL_SETERR((CEL_ERR_LIB, _T("Want state %d undefined."), ha_grp->self->want_state));
        return -1;
    }
...
int cel_hagroup_check_state(CelHaGroup *ha_grp, CelHaState *state, struct timeval *now)
{
    ...
    case CEL_HA_STATE_STANDYBY:
        ...
        break;
    default:
        CEL_SETERR((CEL_ERR_LIB, _T("Ha group %d want state %d undefined."), 
                     ...
                     TCHAR **members, int n_members)
{
    ...
    {
        CEL_SETERR((CEL_ERR_LIB, _T("Bad virtual ha_grp id %d."), id));
        return -1;
    }
    ...
    for (i = 0; i < n_members && i < CEL_HA_MEMBER_NUM && members[i] != NULL; i++)
    {
        ...
        {
            CEL_SETERR((CEL_ERR_LIB, _T("Bad member %s."), members[i]));
            return -1;
        }
    ...
    if (ha_grp->self == NULL)
    {
        CEL_SETERR((CEL_ERR_LIB, _T("No self members.")));
        return -1;
    }
    ha_grp->active = NULL;
    ha_grp->next = NULL;
