...
int cel_httpclient_reading_recv_request(CelHttpClient *client, CelHttpRequest *req)
{
	...
	{
		CEL_SETERR((CEL_ERR_LIB, _T("cel_httprequest_reading error(%s)"), cel_geterrstr()));
		return -1;
	}
	if (cel_stream_get_remaining_length(s) > 0)
	{
...
int cel_httpclient_writing_send_requset(CelHttpClient *client, CelHttpRequest *req)
{
	...
	if (cel_stream_get_remaining_length(s) == 0)
	{
		...
		{
			CEL_SETERR((CEL_ERR_LIB, _T("cel_httprequest_writing error(%s)"), cel_geterrstr()));
			return -1;
		}
		cel_stream_set_position(s, 0);
	}
                                    ...
                                    CelStream *s, CelAsyncResult *result)
{
    ...
    {
        //CEL_SETERR((CEL_ERR_LIB, _T("cel_httpclient_do_send_requset return -1(%s)")));
        result->ret = -1;
...
int cel_httpclient_reading_recv_response(CelHttpClient *client, CelHttpResponse *rsp)
{
	...
	{
		CEL_SETERR((CEL_ERR_LIB, _T("cel_httpresponse_reading error(%s)"), cel_geterrstr()));
		return -1;
	}
    if (cel_stream_get_remaining_length(s) > 0)
	{
...
int cel_httpclient_writing_send_response(CelHttpClient *client, CelHttpResponse *rsp )
{
    ...
    if (cel_stream_get_remaining_length(s) <= 0)
    {
        ...
        {
                CEL_SETERR((CEL_ERR_LIB, _T("cel_httpresponse_writing error(%s)"), cel_geterrstr()));
                return -1;
        }
        cel_stream_set_position(s, 0);
    }
                                 ...
                                 CelHttpExecuteCallbackFunc async_callback)
{
    ...
    if (!cel_httpclient_is_connected(client))
    {
        ...
        if (cel_tcpclient_set_ssl(&(client->tcp_client),use_ssl) != 0
            ...
            || (port = cel_httprequest_get_url_port(req)) == 0)
        {
            CEL_SETERR((CEL_ERR_LIB, _T("cel_httpclient_async_execute error(%s)"), cel_geterrstr()));
            return -1;
        }
