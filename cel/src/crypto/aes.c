 ...
 */
#include "cel/crypto/aes.h"
#include "cel/crypto/_padlock.h"
#include "cel/byteorder.h"

#if defined(CEL_PADLOCK)
static int aes_padlock_ace = -1;
#endif

#if defined(_CEL_AES_ROM_TABLES)
/*
 * Forward S-box
 */
...
static void cel_aes_gen_tables(void)
{
    int i, x, y, z;
    int pow[256];
    int log[256];
 ...
 */
int cel_aes_setkey_enc(CelAesContext *ctx, const BYTE *key, unsigned int keysize)
{
    unsigned int i;
    unsigned long *RK;
    ...
    switch (keysize)
    {
        ...
        default : return (-1/*POLARSSL_ERR_AES_INVALID_KEY_LENGTH*/);
    }

#if defined(CEL_PADLOCK) && defined(PADLOCK_ALIGN16)
    if (aes_padlock_ace == -1)
        aes_padlock_ace = cel_padlock_supports(PADLOCK_ACE);
 ...
 */
int cel_aes_setkey_dec(CelAesContext *ctx, const BYTE *key, unsigned int keysize)
{
    int i, j;
    CelAesContext cty;
    unsigned long *RK;
    ...
    ret = cel_aes_setkey_enc(&cty, key, keysize);
    if (ret != 0)
        return (ret);
    ...
    *RK++ = *SK++;
    ...
    memset(&cty, 0, sizeof(CelAesContext));
 ...
 */
int cel_padlock_xcryptecb(CelAesContext *ctx,
                          int mode,
                          const BYTE input[16],
                          BYTE output[16])
{
 ...
 */
int cel_aes_crypt_ecb(CelAesContext *ctx,
                      int mode,
                      const BYTE input[16],
                      BYTE output[16])
{
    ...
    if (aes_padlock_ace)
    {
        if (cel_padlock_xcryptecb(ctx, mode, input, output) == 0)
            return (0);
    ...
    CEL_GET_LE32(X0, input,  0); X0 ^= *RK++;
    CEL_GET_LE32(X1, input,  4); X1 ^= *RK++;
    CEL_GET_LE32(X2, input,  8); X2 ^= *RK++;
    CEL_GET_LE32(X3, input, 12); X3 ^= *RK++;

    if (mode == CEL_AES_DECRYPT)
    {
    ...
    CEL_PUT_LE32(X0, output,  0);
    CEL_PUT_LE32(X1, output,  4);
    CEL_PUT_LE32(X2, output,  8);
    CEL_PUT_LE32(X3, output, 12);
 ...
 */
int padlock_xcryptcbc(CelAesContext *ctx,
                       int mode,
                       size_t length,
                       BYTE iv[16],
                       const BYTE *input,
                       BYTE *output)
{
    ...
    if(((long) input  & 15 ) != 0 
        || ((long) output & 15 ) != 0)
        return (-1/*POLARSSL_ERR_PADLOCK_DATA_MISALIGNED*/);
 ...
 */
int cel_aes_crypt_cbc(CelAesContext *ctx,
                      int mode,
                      size_t length,
                      BYTE iv[16],
                      const BYTE *input,
                      BYTE *output)
{
    ...
    if (length % 16)
        return (-1/*POLARSSL_ERR_AES_INVALID_INPUT_LENGTH*/);
    ...
    if (mode == CEL_AES_DECRYPT)
    {
        while (length > 0)
        {
            ...
            cel_aes_crypt_ecb(ctx, mode, input, output);
    ...
    else
    {
        while (length > 0)
        {
            ...
            cel_aes_crypt_ecb(ctx, mode, output, output);
            memcpy(iv, output, 16);
...
#if defined(_CEL_AES_MODE_CFB)
/*
 * AES-CFB128 buffer encryption/decryption
 */
int cel_aes_crypt_cfb128(CelAesContext *ctx,
                         int mode,
                         size_t length,
                         size_t *iv_off,
                         BYTE iv[16],
                         ...
                         BYTE *output)
{
    ...
    if (mode == CEL_AES_DECRYPT)
    {
        ...
        {
            if (n == 0)
                cel_aes_crypt_ecb(ctx, CEL_AES_ENCRYPT, iv, iv);
...
#endif /*_CEL_AES_MODE_CFB */

#if defined(_CEL_AES_MODE_CTR)
/*
 * AES-CTR buffer encryption/decryption
 */
int cel_aes_crypt_ctr(CelAesContext *ctx,
                      size_t length,
                      size_t *nc_off,
                      BYTE nonce_counter[16],
                      BYTE stream_block[16],
                      const BYTE *input,
                      BYTE *output)
{
    int c, i;
    ...
    while (length--)
    {
        if (n == 0)
        {
            cel_aes_crypt_ecb(ctx, CEL_AES_ENCRYPT, nonce_counter, stream_block);
