...
{
    memset(ctx, 0, sizeof(CelRsaContext));
...
int cel_rsa_check_privkey(const CelRsaContext *ctx)
{
    ...
    if ((ret = cel_rsa_check_pubkey(ctx)) != 0)
        return (ret);
 ...
 */
static void mgf_mask(BYTE *dst, size_t dlen, BYTE *src, size_t slen,
                     md_context_t *md_ctx)
{
    BYTE mask[POLARSSL_MD_MAX_SIZE];
    BYTE counter[4];
    BYTE *p;
    unsigned int hlen;
    size_t i, use_len;
                          ...
                          BYTE *output)
{
    ...
    return ((mode == RSA_PUBLIC)
            ? cel_rsa_public( ctx, output, output)
            : cel_rsa_private(ctx, output, output));
}
                          ...
                          size_t output_max_len)
{
    ...
    BYTE buf[1024];
#if defined(POLARSSL_PKCS1_V21)
    BYTE lhash[POLARSSL_MD_MAX_SIZE];
    unsigned int hlen;
    const md_info_t *md_info;
    md_context_t md_ctx;
#endif
    ...
    ret = (mode == RSA_PUBLIC)
          ? cel_rsa_public( ctx, input, buf)
          : cel_rsa_private(ctx, input, buf);
                       ...
                       BYTE *sig)
{
    ...
    BYTE *p = sig;
#if defined(POLARSSL_PKCS1_V21)
    BYTE salt[POLARSSL_MD_MAX_SIZE];
    ...
    return ((mode == RSA_PUBLIC)
            ? cel_rsa_public( ctx, sig, sig)
            : cel_rsa_private(ctx, sig, sig));
}
                         ...
                         BYTE *sig)
{
    ...
    BYTE buf[1024];
#if defined(POLARSSL_PKCS1_V21)
    BYTE result[POLARSSL_MD_MAX_SIZE];
    BYTE zeros[8];
    unsigned int hlen;
    size_t slen, msb;
    const md_info_t *md_info;
    ...
    ret = (mode == RSA_PUBLIC)
          ? cel_rsa_public( ctx, sig, buf)
          : cel_rsa_private(ctx, sig, buf);
